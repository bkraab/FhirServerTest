			...
			for(int i = 0; i < resConfigs.size(); i++)
			{
				try
				{
					...
					{	
						...
						for(Object obj: (ArrayList<?>)sourceData) {
							...
							Document newSource = (Document)(obj);
...
//						}
					...
					{
						targetMethod = getMethodForDate(target, detail.getSetMethod());
	...
	private Method getMethodForDate(Object target, String methName)
	{
		Method retMeth = null;
		try
		{		
			boolean found = false;
			Class<?> targetClass = target.getClass();
			Method m = targetClass.getDeclaredMethod(methName, Date.class);
			retMeth = m;
			Method [] allMethods = targetClass.getMethods();			
//			for(Method meth :allMethods)
//			{
